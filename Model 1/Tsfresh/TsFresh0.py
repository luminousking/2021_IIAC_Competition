...
# y = pd.read_csv('\\load_robot_execution_failures_y.csv')
...
# plt.show()
df = pd.read_excel(r'\tsdata_equal.xlsx')
#加入时间戳
df_ts=df.copy()
df_ts["Time"] = None
g = df_ts.groupby('ID')
...
df_ts.to_excel(r'\tsdata_带时间.xlsx',index=False,encoding='gb18030')

df=df_ts.copy()

#描述性统计
df_des=df.describe()
df_des.to_excel(r'\ts_描述性统计.xlsx',encoding='gb18030')
...
df.to_excel(r'\\Tsfreshts_处理后.xlsx',encoding='gb18030',index=False)
##################tiqu ########################
#
df=pd.read_excel(r'\\Tsfreshts_处理后.xlsx')
print(type(df))
print(df.dtypes)
...
def tiqu(i):
    ...
    X_filtered.to_excel(r'\Tsfresh提取特征_'+i+'.xlsx')
    y.to_excel(r'\Tsfresh标签_'+i+'.xlsx')
...
extract_settings = ComprehensiveFCParameters()
X = extract_features(dfa, column_id='ID', column_sort='Time',default_fc_parameters=extract_settings, impute_function=impute)
X.to_excel(r'\Tsfresh全部特征_中部跳动.xlsx')
...
# X_train, X_test, X_filtered_train, X_filtered_test, y_train, y_test = train_test_split(X, X_filtered, y, test_size=4)
X_filtered.to_excel(r'\Tsfresh提取特征_中部跳动.xlsx')
y.to_excel(r'\Tsfresh标签_中部跳动.xlsx')

print(X.shape)
print(y.shape)
print(dfa.shape)

df8y=pd.read_excel(r'Tsfresh标签_中部跳动.xlsx')
df8x=pd.read_excel(r'\Tsfresh全部特征_中部跳动.xlsx')
df8=pd.merge(df8y, df8x, how='inner', left_on='ID',right_on='Unnamed: 0')
df8.to_excel(r'Tsfresh回归数据_8.xlsx')


df6y=pd.read_excel(r'\Tsfresh标签_Q处跳动≤0.6.xlsx')
df6x=pd.read_excel(r'\Tsfresh全部特征_Q处跳动≤0.6.xlsx')
df6=pd.merge(df6y, df6x, how='inner', left_on='ID',right_on='Unnamed: 0')
df6.to_excel(r'\Tsfresh回归数据_6.xlsx')